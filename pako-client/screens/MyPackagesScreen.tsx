...
const MyPackagesScreen: React.FC<MyPackagesScreenProps> = ({ navigation }) => {
  ...
  const loadOrders = async () => {
    try {
      ...
      console.log('=== DEBUG MyPackagesScreen ===');
      ...
      console.log('Statuts des commandes:', orders.map((o: any) => o.status));
  ...
  const renderOrderItem = ({ order }: { order: any }) => {
    const isInProgress = order.status === 'confirmed' || order.status === 'in_transit' || order.status === 'pending';
      ...
      <View style={styles.historyCard}>
        ...
        >
          ...
          <View style={styles.historyHeader}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: isInProgress ? '#FF9800' : '#4CAF50' }]}>
              <Text style={styles.statusText}>{isInProgress ? 'En cours' : 'Livré'}</Text>
            </View>
          </View>
            ...
            {loading ? (
              ...
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>
                  Créez votre première commande pour voir vos colis ici
                </Text>
              </View>
            ) : (
              <>
